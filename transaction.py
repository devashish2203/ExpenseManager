...
class Pattern(Enum):
    CITIBANK_CC = "Rs. (?P<amount>[\d,]+\.\d{2}).*(?P<cardType>card).*(?P<cardNo>\d{4}) on (?P<date>.*) at (?P<merchant>.*?\.)"
    CITIBANK_CC_OLD = "Rs (?P<amount>[\d,]+\.\d{2}).*(?P<cardType>Credit).*(?P<cardNo>\d{4}X{8}\d{4}) on (?P<date>.*) at (?P<merchant>.*)"
